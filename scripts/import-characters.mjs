#!/usr/bin/env node
// One-shot script: imports all characters from lib/characters.ts into Sanity
// Run once: node scripts/import-characters.mjs
// Requires env: NEXT_PUBLIC_SANITY_PROJECT_ID, SANITY_API_TOKEN
...
const characters = [
  ...
  },
  {
    ...
    lead: "Rick Sanchez est un agent fédéral obsédé par le démantèlement des réseaux criminels de Leonida. Brillant et sans scrupule quand il s'agit d'obtenir des résultats, il représente la pression légale permanente sur Jason et Lucia — toujours un coup d'avance, jamais vraiment hors de portée.",
    histoire: [
...
async function main() {
  ...
  console.log(`Importing ${characters.length} characters into Sanity...`)
  ...
  for (const char of characters) {
    const doc = {
      ...
      order: char.order,
      // coverImage & mediaImages: images locales — à uploader manuellement dans le Studio
    }
  ...
  console.log('\nDone! All characters are now in Sanity.')
  console.log('Note: cover images and galleries are still local (/images/*.jpg).')
  console.log('Upload official images via the Sanity Studio when available.')
}
